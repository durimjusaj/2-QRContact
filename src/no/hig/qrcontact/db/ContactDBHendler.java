...
public class ContactDBHendler extends SQLiteOpenHelper {
	...
	public ContactDBHendler(Context context) {
	...
	public void onCreate(SQLiteDatabase db) {
		String CREATE_RSS_TABLE = "CREATE TABLE IF NOT EXISTS "
				...
				+ KEY_NAME + " TEXT," + KEY_SURNAME + " TEXT," + KEY_NUMBER
				+ " TEXT," + KEY_EMAIL + " TEXT," + KEY_SHARE + " INTEGER"
				+ ")";
	...
	public boolean addContact(Contact m) {
		...
		if (!isContactExists(db, m.getNumber())) {
			result = (db.insert(TABLE_CONTATCS, null, values) > 0 ? true
					: false);
			db.close();
		} else {
			result = false;
	...
	public boolean isContactExists(SQLiteDatabase db, String nr) {

		Cursor cursor = db.rawQuery("SELECT 1 FROM " + TABLE_CONTATCS
				+ " WHERE number = '" + nr + "'", new String[] {});
